...
# Load Data
df_KICH_KIRC = pd.read_csv(f"./csvs/ExpressionLevels_KICH_KIRC_Labelled.csv", sep="\t",header=None)
df_KICH_KIRP = pd.read_csv(f"./csvs/ExpressionLevels_KICH_KIRP_Labelled.csv", sep="\t", header=None)
df_KIRC_KIRP = pd.read_csv(f"./csvs/ExpressionLevels_KIRC_KIRP_Labelled.csv", sep="\t", header=None)

def analyze_tumor_pair(csv_file_path, return_stats=False):

    # 1. Read CSV (Default separator to avoid errors)
       # first column contains tumor labels (no header) — read without header
        # header exists but first header cell might be blank — read with header and rename first column
    df_raw = pd.read_csv(csv_file_path, header=0)
    # ensure the first column is named 'Tumor_Type' (handles blank header)
    df_raw.rename(columns={df_raw.columns[0]: 'Tumor_Type'}, inplace=True)


    tumor_types = df_raw['Tumor_Type'].unique()
    ...
    X = df_raw.drop('Tumor_Type', axis=1)
    y = df_raw['Tumor_Type']
...
pair1, score1 = analyze_tumor_pair(
    './csvs/ExpressionLevels_KICH_KIRC_Labelled.csv',)

all_results.append({"Pair": pair1, "Score": score1})

# Pair 2: KICH vs KIRC
pair2, score2, pvals_target, X_target, y_target = analyze_tumor_pair('./csvs/ExpressionLevels_KICH_KIRP_Labelled.csv', return_stats=True)
all_results.append({"Pair": pair2, "Score": score2})

# Pair 3: KICH vs KIRP
pair3, score3 = analyze_tumor_pair('./csvs/ExpressionLevels_KIRC_KIRP_Labelled.csv')
